...
import { Car, Users, DollarSign, Wrench, TrendingUp, AlertTriangle, ClipboardList } from "lucide-react";
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{metrics.checklistsHoje}</div>
          <p className="text-xs text-muted-foreground">
            Execuções realizadas
          </p>
          {metrics.divergenciasPendentes > 0 && (
            <div className="mt-2 flex items-center text-xs text-orange-600">
              <AlertTriangle className="h-3 w-3 mr-1" />
              {metrics.divergenciasPendentes} divergências pendentes
            </div>
          )}
        </CardContent>
      </Card>
